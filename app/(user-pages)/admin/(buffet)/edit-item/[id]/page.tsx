...
function EditItem({ params }) {    
    ...
    useEffect(() => {
        if (!fetchItemLoading && !error) setUpdatedItemInfo(oldItemInfo);
        else if (!fetchItemLoading && error) notFound();
    }, [fetchItemLoading, oldItemInfo]);
    ...
    return (
        <>
            <DynamicTitle title={`تعديل ${oldItemInfo.name}`}  />
            {isPopupOpened && (
                <Popup
                    type='success'
